...
from t26_1_a import A as A1
from t26_1_b import A as A2
from t26_1_c import A as A3
from t26_1_gloss import GLOSS
...
SOURCE = RAW.get('source') or (
    'https://english-exam.lazynote.cn/kaoyan/sections/%d-english-two/'
    'section2-part-a-%d/' % (YEAR, TEXT))
# 2026 Text 1 的 PDF 页脚未印分段号（section2-part-a/ 而非 -1/），
# 故不能按 TEXT 硬拼，改从 source 推导：sections→paper、去掉查询串。
SENT_BASE = re.sub(r'[?#].*$', '', SOURCE).replace('/sections/', '/paper/')
if not SENT_BASE.endswith('/'):
    SENT_BASE += '/'
